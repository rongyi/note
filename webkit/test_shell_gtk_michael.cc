...
gboolean MainWindowExpose(GtkWidget* widget,
...
bool TestShell::Initialize(const GURL& starting_url) {
  ...
  gtk_widget_set_app_paintable(GTK_WIDGET(m_mainWnd), TRUE);
  ...
  g_signal_connect(GTK_WIDGET(m_mainWnd), "expose-event",
                     G_CALLBACK(MainWindowExpose), this);
  ...
  GdkScreen *screen = gtk_widget_get_screen(GTK_WIDGET(m_mainWnd));
  ...
  if (colormap) {
      ...
      gtk_widget_set_colormap(GTK_WIDGET(m_mainWnd), colormap);
    } else {
