...
class WebWidgetHostGtkWidget {
 public:
  ...
  static GtkWidget* CreateNewWidget(GtkWidget* parent_view,
    ...
    //                  G_CALLBACK(&HandleScreenChanged), host);
    ...
    // bool isComposited = gdk_screen_is_composited(screen);
 ...
 private:
  ...
  // static void HandleScreenChanged(GtkWidget* widget,
  ...
  static gboolean HandleExpose(GtkWidget* widget,
    ...
    cairo_t* cairo_drawable = gdk_cairo_create(widget->window);
    
    // cairo_surface_t * win_surface_cairo_drawable = cairo_get_target(cairo_drawable);     
    // cairo_surface_write_to_png(win_surface_cairo_drawable,"win_surface_cairo_drawable.png");

    if (rgba) {
        cairo_set_source_rgba (cairo_drawable, 1.0, 1.0, 1.0, 0.0);
    } else {
        cairo_set_source_rgba (cairo_drawable, 1.0, 1.0, 1.0, 1.0);
    }

    // cairo_surface_t * win_surface_cairo_drawable_alpha = cairo_get_target(cairo_drawable);     
    // cairo_surface_write_to_png(win_surface_cairo_drawable_alpha,"win_surface_cairo_drawable_alpha.png");

    cairo_set_operator (cairo_drawable, CAIRO_OPERATOR_SOURCE);
...
void WebWidgetHost::Paint() {
  ...
  if (!canvas_.get()) {
    ...
    canvas_.reset(new skia::PlatformCanvas(width, height, false));
//    canvas_.reset(new skia::PlatformCanvas(width, height, false)); //russel modify change tue to false
    if (!canvas_.get()) {
  ...
  GdkScreen *screen = gtk_widget_get_screen(view_);
  GdkColormap *rgba = gdk_screen_get_rgba_colormap (screen); 

    // cairo_surface_t * web_surface_cairo_drawable = cairo_get_target(cairo_drawable);     
    // cairo_surface_write_to_png(web_surface_cairo_drawable,"web_surface_cairo_drawable.png");
  
    // cairo_surface_t * source_surface_cairo_drawable = cairo_get_target(source_surface);     
    // cairo_surface_write_to_png(source_surface_cairo_drawable,"source_surface_cairo_drawable.png");

  
  if (rgba) {
     gtk_widget_set_colormap(GTK_WIDGET(view_), rgba);
     gtk_widget_realize(GTK_WIDGET(view_));
     cairo_set_source_rgba (cairo_drawable, 1.0, 1.0, 1.0, 0.0);
     cairo_set_operator (cairo_drawable, CAIRO_OPERATOR_SOURCE);
  }

  //  cairo_surface_t * web_surface_cairo_drawable_alpha = cairo_get_target(cairo_drawable);     
  // cairo_surface_write_to_png(web_surface_cairo_drawable_alpha,"oweb_surface_cairo_drawable_alpha.png");
